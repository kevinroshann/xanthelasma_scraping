...
def get_images_from_google(wd, delay, max_images):
    ...
    url = "https://www.bing.com/images/search?q=xanthelasma&first=1&cw=1177&ch=771"
    wd.get(url)
    ...
    while len(image_urls) < max_images:
        ...
        if new_height == last_height:
            try:
                ...
                load_more_button = wd.find_element(By.XPATH, "//button[text()='See more images']")
                load_more_button.click()
                time.sleep(delay)
            except:
                try:
                    # Click "Load More" button using partial link text (attempt 2)
                    more_results_link = wd.find_element(By.PARTIAL_LINK_TEXT, "See more images")
...
for i, url in enumerate(urls):
    download_image("imgsedge/", url, str(i) + ".jpg")
