...
def get_images_from_google(wd, delay, max_images):
    ...
    url = "https://yandex.com/images/search?from=tabbar&text=xanthelasma"
    wd.get(url)
    ...
    while len(image_urls) < max_images:
        ...
        if new_height == last_height:
            try:
                ...
                load_more_button = wd.find_element(By.XPATH, "//button[text()='Show more']")
                load_more_button.click()
                time.sleep(delay)
            except:
                try:
                    # Click "Load More" button using partial link text (attempt 2)
                    more_results_link = wd.find_element(By.PARTIAL_LINK_TEXT, "Show more")
        ...
        for image in images:
            ...
            if image.get_attribute('src') and 'http' in image.get_attribute('src'):
                ...
                download_image("imgsyandex/", image.get_attribute('src'), str(len(image_urls)) + ".jpg")
...
for i, url in enumerate(urls):
    download_image("imgsyandex/", url, str(i) + ".jpg")
